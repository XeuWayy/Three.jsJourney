...
// Shaders import
import vertexFirework from './shaders/fireworks/vertex.glsl'
import fragmentFirework from './shaders/fireworks/fragment.glsl'
...
const firework = (count, position, size, texture, radius, color) => {
    ...
    const material = new THREE.ShaderMaterial({
        vertexShader: vertexFirework,
        fragmentShader: fragmentFirework,
        uniforms: {
            uSize: new THREE.Uniform(size),
            uViewPort: new THREE.Uniform(sizes.resolution),
            uPixelRation: new THREE.Uniform(sizes.pixelRatio),
...
const createRandomFirework  = () => {
    ...
    const radius = 0.75 + Math.random()
    const color = new THREE.Color()
    color.setHSL(Math.random(),1, 0.7)
    firework(count, position, size, texture, radius, color)
}
...
gui.add(effectController, 'elevation', -3, 90, 0.01).onChange(updateSky);
